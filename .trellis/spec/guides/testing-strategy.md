...
- 图片处理决策：5 MB 以内的 JPG/PNG/WebP 原图、带 alpha 的 PNG、Logo/二维码目标策略、单次编码、预览元数据、恰好达到/超过最终 128 KB/1 MB 限制、对象 URL 清理，以及压缩后二维码由真实解码器验收
- 错误映射和脱敏 helper
...
- multipart 请求体与最终文件字节限制、PNG 签名/chunk/IHDR 尺寸/宽高/总像素、用途、完整 workerd 解码和生成的 R2 key
...
7. 将带透明度的 PNG 转为保留 alpha 的 PNG，在 Logo 限制内上传，并拒绝超限结果；压缩二维码后用真实扫码器确认仍可识别且为白底不透明 PNG。
