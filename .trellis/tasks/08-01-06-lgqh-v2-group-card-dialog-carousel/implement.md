...
## 17. T03 迁移基线后的执行顺序（有效计划）

1. 读取并审计 `08-02-t03-visual-migration` 的完成记录、正式组件路径、交接清单和迁移差异。
2. 检查正式组件没有 import `prototype/`、固定 fixture、模拟业务状态或原型存储。
3. 核对 T04 的公开群组 DTO、状态过滤、显示宽度和现有公开详情 API。
4. 冻结 GroupCard/Carousel/Dialog 的正式 props、events、slots、composable 和 Router owner，不再设计新 UI。
5. 先写真实 DTO、公开投影、详情读取、错误映射和加载/重试测试。
6. 将真实摘要/详情数据接入现有正式组件，保持 T02/T03 的视觉结构和无障碍行为。
7. 复用现有点赞、设备身份、QR、复制、分享和 Toast；补充失败回滚与安全 URL 测试。
8. 接入 `group` controller、q 共存、push/replace、深链接、前进/后退和关闭焦点恢复。
9. 执行公开过滤、非公开 ID、协议校验、错误脱敏、缓存/竞态和移动端回归。
10. 将正式组件消费契约和测试证据交接给 T07，并把跨任务问题交给 T10。

### 17.1 停止条件

- T03 迁移结果缺少正式组件路径、API 或焦点/滚动交接。
- 真实 DTO 需要新增领域模型、数据库 migration 或改变 T04/T05 契约。
- 迁移组件需要重新设计页面结构，而不是修复数据接入或兼容问题。
- prototype 逻辑被带入正式运行时，或视觉回归无法区分 T02 基线与 T03 迁移差异。
