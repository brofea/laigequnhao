...
11. 执行空库、旧库、超限数据、失败路径和部署顺序 migration 演练。
12. 按最小修复原则处理跨任务阻塞问题。
...
- [ ] fixture 对旧库 migration 和新库初始化分别有版本标识。
...
- [ ] 建立包含已发布/下架/回收站群组、标签、加入方式、图片、点赞和版本字段的旧库。
- [ ] 加入相同创建时间和可能超限旧文本。
- [ ] 应用新 migration，记录实际命令和退出码。
- [ ] 验证旧数据数量、内容、资源引用和状态保留。
- [ ] 验证已发布群组 `last_published_at` 按 T04 策略回填，不统一使用部署时间。
...
- [ ] 验证标题/简介超限数据不被静默截断。
- [ ] 验证公开和管理读取超限旧数据不崩溃。
- [ ] 验证编辑其他字段和修改超限字段符合批准策略。
- [ ] 模拟 migration 中途失败、索引失败和回填异常。
...
- [ ] 空库、旧库、超限数据、失败和部署顺序 migration 演练有记录。
- [ ] 所有跨任务修复均有最小变更和回归测试。
