...
- 父任务：`lgqh-v2`。
- 当前状态：`completed`。
- 当前阶段：已完成 `research.md`、`impact-map.md`、项目 Spec 和受影响规划修订；不修改业务代码、测试或正式 migration。
- 未来审计仍不得修改业务代码、migration、测试代码、快照、Tailwind 配置或依赖。
- 已确认的产品决策“已下架群组完全不公开”作为后续所有差异表、所有权和阻塞判断的硬约束，不再保留旧公开行为作为实现选项。
- 视觉模板依赖暂缓：T01 不等待模板即可继续审计；进入 T02 前由用户补充模板并更新 `ui-design.md` / 前端 Spec，T02 在此之前不得冻结视觉规范或进入实施。
...
6. 读取当前已创建的 T02–T09 子任务规划，核对其依赖、所有权和启动门槛。
7. 记录缺失、不可读取、命名与源 PRD 不同的材料；特别记录 T02 所需视觉模板当前未在仓库提供。
...
当前阶段已获用户批准并完成 T01 审计产物；父任务、项目 Spec 和受影响源 PRD 已按证据修订，业务代码、测试和正式 migration 全程保持只读。
...
- 在差异表中单列当前质量基线失败，记录命令、失败文件、是否为历史漂移、建议归属和是否阻塞下一阶段；不得在 T01 中直接修复。
- 已确认的基线处置为：T04 接管共享 `platforms` Contract 与 `config.spec.ts`；T10 接管暂存资源清理、图片处理器文案、`scripts/seed-local.mjs` lint 和 22 个格式失败。后续任务必须提供根因、最小修复、自动化回归和复测结果。
- PRD 修订未遗漏已冻结产品规则。
- 子任务影响图没有遗漏高冲突共享文件。

## 8. 建议验证命令

这些命令用于只读审计和工作区证明；即使 T01 已进入 `in_progress`，也不代表进入 V2 业务实现：
