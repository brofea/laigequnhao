...
**已批准：自研零依赖**。做小型 spike，不接入生产路径：

- 在共享模块建立固定 Unicode 版本的自研 EAW/Emoji 范围表，`Intl.Segmenter` 作为分段主引擎，手写 UAX#29 子集 fallback 分段器作对照兜底。
- 对 grapheme、EAW、Emoji/ZWJ、空白、Tab、换行、CRLF、控制字符运行 golden vectors。
- 在 Node、Workers、浏览器三端断言同一向量集结果一致；任何差异通过手写 fallback 对照定位，不能以运行时差异放行。
- 不引入任何 Unicode 处理依赖；记录表体积基线（预计 8–12KB 范围数据）。
...
- 移除/调整与显示宽度冲突的 `maxlength`（`AdminGroupFields.vue` 的 title/description、`SubmissionDialog.vue` 的 title/description），保留字段级错误展示。
- 复用共享测量 helper 与 Contract 错误语义；不新增宽度计数器、不写 IME composition 逻辑（计数器与视觉由后续 UI 任务负责）。
- 检查并更新相关组件 spec（`SubmissionDialog.spec.ts` 等）；T03 工作树已修改部分组件，编辑前先 Read 最新内容。
...
- [ ] 明确本次 schema/config 变更对 T03 主题配置、真实表单和前端 API 消费的影响。
- [ ] 冻结并校验 T03 的标题/品牌、GitHub URL/文案、添加新群文案/入口配置；默认标题为“来个群号”，默认 GitHub URL 为 `https://github.com/brofea/laigequnhao`，默认添加入口为现有提交弹窗。
- [ ] 真实 API/共享 Contract 的变更有兼容测试和前端交接记录；未用 `prototype/` Mock 替代。
- [ ] 未修改 T03 的视觉实现；发现前端阻塞时记录 owner 和依赖，不越界修复。
