...
- T05 不修改 T03 主题/Token/顶栏；板块 API 必须按正式前端真实接入要求提供稳定的 Zod Contract、认证/CSRF、version/mutation token、公开 published-only 投影和可展示错误。
- T05 交接给正式前端的页面不得重新硬编码站点标题、品牌、GitHub 或添加新群入口；这些展示配置由 T03/T04 的站点配置提供，T05 只交付业务 API/Contract。
- T05 交付必须说明 T07/T08 如何消费真实 API，禁止使用 `prototype/` 固定数据替代 route/service/Workers 证据；公开与管理数据流必须区分。
