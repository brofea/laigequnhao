...
<template>
  <div class="board-manager">
    <div class="board-manager__toolbar">
      ...
      <PrototypeButton variant="normal" size="sm" icon="plus" @click="emit('addBoard')"
        >添加板块</PrototypeButton
      >
    </div>
    ...
    <div class="board-list">
      ...
      >
        <header class="board-panel__header">
          ...
          <PrototypeBadge :tone="board.enabled ? 'success' : 'neutral'" dot>{{
        ...
        <div v-if="expandedId === board.id" class="board-panel__content">
          <div v-if="confirmDeleteId === board.id" class="delete-confirm" role="alert">
            ...
            <span class="delete-confirm__actions">
              ...
              >
                >确认删除</PrototypeButton
              >
              <PrototypeButton variant="quiet" size="sm" @click="confirmDeleteId = null"
                >取消</PrototypeButton
              >
            </span>
          </div>
          <div v-else class="board-members-toolbar">
            ...
            <PrototypeButton variant="quiet" size="sm" icon="plus" @click="emit('addGroup', board)"
              >添加新群</PrototypeButton
            >
          </div>
          <div v-if="board.members.length" class="board-members">
            <table>
              <caption class="proto-sr-only">
                {{
                  board.title
                }}成员
              </caption>
              ...
              <tbody>
                <tr v-for="memberId in board.members" :key="memberId">
                  ...
                  <td class="board-members__status">
                    <PrototypeBadge :tone="memberStatusTone(memberId)" dot>{{
                  ...
                  <td>
                    <div class="board-member-order-actions">
                      ...
                      >
                        <PrototypeIcon name="chevron-up" size="14" />
                      </button>
                      <button
                        class="table-icon-button"
                        type="button"
                        ...
                        @click="moveMember(board.id, memberId, 1)"
                      >
                        <PrototypeIcon name="chevron-down" size="14" />
                      </button>
                    </div>
                  </td>
                  <td>
                    <div class="board-member-actions">
                      ...
                      >
                        <PrototypeIcon name="edit" size="14" />编辑
                      </button>
                      <button
                      ...
                      >
                        <PrototypeIcon name="arrow-right" size="14" />移除
                      </button>
                    </div>
                    <button
                      ...
                      @click="editMember(board, memberId)"
                    >
                      <PrototypeIcon name="more" size="18" />
                    </button>
                  </td>
                </tr>
              </tbody>
          ...
          <div v-else class="proto-empty proto-empty--compact">
