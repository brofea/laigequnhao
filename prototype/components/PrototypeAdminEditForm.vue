...
<template>
  <form class="admin-edit-form" @submit.prevent="save">
    <div v-if="!props.publicMode" class="admin-edit-form__status">
      <PrototypeBadge :tone="props.publicMode ? 'warning' : groupStatusTones[draft.status]" dot>{{
    ...
    <section class="admin-edit-section">
      ...
      <div class="admin-edit-avatar-row">
        ...
        <div class="admin-edit-inline-actions">
          <PrototypeButton variant="normal" size="sm" @click="openAvatarPicker"
            >上传头像</PrototypeButton
          ><PrototypeButton variant="quiet" size="sm" @click="emit('toast', '已模拟移除头像')"
            >移除</PrototypeButton
          >
          <input
            ref="avatarInput"
      ...
      <div class="admin-edit-fields-grid">
        ...
        <div v-else class="public-submit-status" aria-label="审核状态">
          <span class="proto-field__label">状态</span>
          <div class="public-submit-status__value">
            <PrototypeBadge tone="warning" dot>待审核</PrototypeBadge
      ...
      <div class="admin-edit-tags">
        <span v-for="tag in draft.tags" :key="tag" class="admin-edit-tag"
          ># {{ tag
            ...
            <PrototypeIcon name="close" size="13" /></button></span
    ...
    <section class="admin-edit-section">
      ...
      <div class="admin-edit-join-list">
        ...
        >
          ...
          <template v-if="method.type === 'qr'">
            <div class="admin-edit-qr-editor">
              ...
              <label
                class="proto-button proto-button--normal proto-button--sm admin-edit-upload-button"
              >
                <input
                  type="file"
                  accept="image/*"
                ...
                />
                <PrototypeIcon name="upload" size="16" />
                <span class="proto-button__label">上传图片</span>
              </label>
        ...
        <div v-if="!draft.joinMethods.length" class="proto-empty proto-empty--compact">
    ...
    <div class="admin-edit-form__footer">
      <PrototypeButton
        ...
        >{{ props.removable ? "移除群组" : "删除群组" }}</PrototypeButton
      >
      <span class="admin-edit-form__footer-spacer"></span>
      <PrototypeButton variant="quiet" @click="emit('cancel')">取消</PrototypeButton
